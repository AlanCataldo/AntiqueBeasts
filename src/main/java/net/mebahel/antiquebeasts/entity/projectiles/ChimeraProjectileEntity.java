...
import static net.mebahel.antiquebeasts.entity.ModEntities.CHIMERA_PROJECTILE;

public class ChimeraProjectileEntity extends ThrownItemEntity implements IAnimatable {
    float damage;
    ...
    public ChimeraProjectileEntity(World world, LivingEntity owner, float damage) {
        super(CHIMERA_PROJECTILE, owner, world);
        this.damage = damage;
    }

    public ChimeraProjectileEntity(EntityType<? extends ChimeraProjectileEntity> entityType, World world) {
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        if (entityHitResult.getEntity() instanceof LivingEntity) {
            ...
            target.damage(DamageSource.thrownProjectile(this, this.getOwner()), 8.0F);
    ...
    public void tick() {
        ...
        if (!this.hasNoGravity()) {
            ...
            this.setVelocity(vec3d2.x, vec3d2.y, vec3d2.z - 0.05); // Slightly adjust Z velocity for a more dynamic movement
        }
        this.setPosition(d, e, f);

        // Create fire trail on the ground
        BlockPos pos = new BlockPos((int) this.getX(), (int) this.getY(), (int) this.getZ());
        if (this.getWorld().getBlockState(pos.down()).isFullCube(this.getWorld(), pos.down()) && this.getWorld().getBlockState(pos).isAir()) {
