...
public class PegasusEntity extends HorseEntity implements IAnimatable, IAnimationTickable {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> animationState) {
        if (transitioningToGround) {
            animationState.getController().setAnimation(new AnimationBuilder().addAnimation("fly_to_ground", ILoopType.EDefaultLoopTypes.PLAY_ONCE));
        ...
        if (transitioningToFly) {
            animationState.getController().setAnimation(new AnimationBuilder().addAnimation("fly_from_ground", ILoopType.EDefaultLoopTypes.PLAY_ONCE));
            if (++transitionFromGroundTicks > 10) {
                transitioningToFly = false;
                transitionFromGroundTicks = 0;
            }
            return PlayState.CONTINUE;
        }

        if (animationState.isMoving() && !this.isGliding) {
            animationState.getController().setAnimation(new AnimationBuilder().addAnimation("idle_to_walk", ILoopType.EDefaultLoopTypes.PLAY_ONCE).addAnimation("walk2", ILoopType.EDefaultLoopTypes.LOOP));
        } else if (animationState.isMoving() && this.isGliding && !transitioningToFly) {
            animationState.getController().setAnimation(new AnimationBuilder().addAnimation("fly", ILoopType.EDefaultLoopTypes.LOOP));
