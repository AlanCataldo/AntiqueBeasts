...
public class DraugrWightMeleeAttackGoal extends Goal {
    ...
    public boolean shouldContinue() {
        LivingEntity target = mob.getTarget();
        if (target instanceof PlayerEntity player && (player.isCreative() || player.isSpectator())) return false;
        return target != null && target.isAlive() && !this.mob.isShooting();
    }
    ...
    public void tick() {
        LivingEntity target = mob.getTarget();
        if (target != null && target.isAlive() && !this.mob.isShooting()) {
    ...
    protected void attack(LivingEntity target) {
        ...
        if (mob.isSwinging() && !hasDashed && cooldown == max_cooldown - 10) {
            hasDashed = true;
