...
public class DraugrWightMeleeAttackGoal extends Goal {
    protected final DraugrWightEntity mob;
    ...
    public DraugrWightMeleeAttackGoal(DraugrWightEntity mob, double speed, int max_cooldown, int damage_time) {
        this.mob = mob;
        this.speed = speed;
    ...
    public boolean canStart() {
        ...
        // Pas de melee si tir de spell
        if (mob.isShooting()) return false;
    ...
    public boolean shouldContinue() {
        ...
        // Le Wight ne melee pas s'il est en phase de tir
        return target != null && target.isAlive() && !mob.isShooting() && this.mob.getHasSpawned();
    }
    ...
    @Override
    public void tick() {
        // ⚠ Si le Wight boit, le melee n'interfère pas
        ...
        // Et si le Wight est en train de tirer, le melee ne fait rien
        if (mob.isShooting()) {
            mob.setSwinging(false);
            return;
        }
