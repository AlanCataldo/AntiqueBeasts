...
public class DraugrRaidDifficultyState extends PersistentState {
    private static int netherCheckCounter = 0;
    private static final int NETHER_CHECK_INTERVAL = 300;
    private int difficultyLevel;
    DraugrRaidDifficultyState antiquebeastsDifficultyState = null;

    public DraugrRaidDifficultyState(int difficultyLevel) {
    ...
    public static DraugrRaidDifficultyState fromNbt(NbtCompound nbt) {
        return new DraugrRaidDifficultyState(nbt.getInt("antiquebeastsDifficultyLevel"));
    }

    public void registerDifficultyState(ServerWorld world) {
        if (DraugrCombatBalancingConfig.draugrRaidScalingDifficulty) {
            PersistentStateManager stateManager = world.getPersistentStateManager();
            this.antiquebeastsDifficultyState = stateManager.getOrCreate( // ✅ Stocke la valeur dans `this.difficultyState`
                    DraugrRaidDifficultyState::fromNbt,
                    () -> new DraugrRaidDifficultyState(1),
    ...
    private static void checkNetherVisit(ServerWorld world, DraugrRaidDifficultyState antiquebeastsDifficultyState) {
